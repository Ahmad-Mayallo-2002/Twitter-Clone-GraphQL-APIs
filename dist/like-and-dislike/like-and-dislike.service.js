"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LikeAndDislikeService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const like_entity_1 = require("./enitites/like.entity");
const typeorm_2 = require("typeorm");
const dislike_entity_1 = require("./enitites/dislike.entity");
const tweet_entity_1 = require("../tweet/entities/tweet.entity");
const user_entity_1 = require("../user/entities/user.entity");
let LikeAndDislikeService = class LikeAndDislikeService {
    likeRepo;
    dislikeRepo;
    userRepo;
    tweetRepo;
    constructor(likeRepo, dislikeRepo, userRepo, tweetRepo) {
        this.likeRepo = likeRepo;
        this.dislikeRepo = dislikeRepo;
        this.userRepo = userRepo;
        this.tweetRepo = tweetRepo;
    }
    async getTweet(tweetId) {
        const tweet = await this.tweetRepo.findOneBy({ id: tweetId });
        if (!tweet)
            throw new common_1.NotFoundException('Tweet is not Found!');
        return tweet;
    }
    async addLike(userId, tweetId) {
        const tweet = await this.getTweet(tweetId);
        const like = await this.likeRepo.findOneBy({ user: { id: userId } });
        const dislike = await this.dislikeRepo.findOneBy({ user: { id: userId } });
        if (dislike)
            await this.dislikeRepo.delete(dislike.id);
        if (like) {
            await this.likeRepo.delete(like.id);
            return true;
        }
        const newLike = this.likeRepo.create({
            user: { id: userId },
            tweet: { id: tweet?.id },
        });
        return await this.likeRepo.save(newLike);
    }
    async addDislike(userId, tweetId) {
        const tweet = await this.getTweet(tweetId);
        const like = await this.likeRepo.findOneBy({ user: { id: userId } });
        const dislike = await this.dislikeRepo.findOneBy({ user: { id: userId } });
        if (dislike)
            await this.dislikeRepo.delete(dislike.id);
        if (like) {
            await this.likeRepo.delete(like.id);
            return true;
        }
        const newDislike = this.dislikeRepo.create({
            user: { id: userId },
            tweet: { id: tweet?.id },
        });
        await this.likeRepo.save(newDislike);
        return true;
    }
    async getLikeTweetCount(tweetId) {
        const counts = await this.likeRepo.count({
            where: { tweet: { id: tweetId } },
        });
        return counts;
    }
    async getDislikeTweetCount(tweetId) {
        const counts = await this.dislikeRepo.count({
            where: { tweet: { id: tweetId } },
        });
        return counts;
    }
};
exports.LikeAndDislikeService = LikeAndDislikeService;
exports.LikeAndDislikeService = LikeAndDislikeService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(like_entity_1.Like)),
    __param(1, (0, typeorm_1.InjectRepository)(dislike_entity_1.Dislike)),
    __param(2, (0, typeorm_1.InjectRepository)(user_entity_1.User)),
    __param(3, (0, typeorm_1.InjectRepository)(tweet_entity_1.Tweet)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository])
], LikeAndDislikeService);
//# sourceMappingURL=like-and-dislike.service.js.map